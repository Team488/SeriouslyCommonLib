...
public abstract class BaseSwerveDriveSubsystem extends BaseDriveSubsystem
    ...
    public void move(XYPair translate, double rotate, XYPair centerOfRotationInches) {
        ...
        if (isNotMoving) {
            ...
            moduleStates = SwerveDriveKinematics.desaturateWheelVelocities(moduleStates, topSpeedMetersPerSecond);
        }

        // Finally, we can tell each swerve module what it should be doing. Log these
        // values for debugging.
        final SwerveModuleVelocity[] finalModuleStates = moduleStates;
        aKitLog.withLogLevel(AKitLogger.LogLevel.INFO, () -> {
            aKitLog.record("DesiredSwerveState", finalModuleStates);
        });
